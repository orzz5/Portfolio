...
export default function ConstellationGrid() {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            // Dark Purple RGB: 88, 28, 135 (#581c87)
            const nodeColor = '88, 28, 135';
            const accentColor = isDarkMode ? '168, 85, 247' : '126, 34, 206'; // Purple Accent
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                for (let j = i + 1; j < nodes.length; j++) {
                    ...
                    if (distSq < MAX_CONN_DIST_SQ) {
                        ...
                        const alpha = (1 - nDist / MAX_CONN_DIST) * (isDarkMode ? 0.25 : 0.15);
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                const baseAlpha = isNear ? 0.95 : 0.6 + Math.sin(n.pulse) * 0.15;
    ...
    return (
        <div className="fixed inset-0 -z-10 w-full h-full overflow-hidden select-none bg-slate-950 dark:bg-slate-950 light:bg-slate-50 pointer-events-none">
